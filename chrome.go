...
// GetBounds defines settable window properties.
type Bounds struct {
	Top         int         `json:"top"`
	Left        int         `json:"left"`
...
func (c *chrome) setBounds(b Bounds) error {
	...
	param := h{"windowId": c.window, "getBounds": b}
	if b.WindowState != WindowStateNormal {
		param["getBounds"] = h{"windowState": b.WindowState}
	}
	_, err := c.send("Browser.setWindowBounds", param)
	return err
}

func (c *chrome) getBounds() (Bounds, error) {
